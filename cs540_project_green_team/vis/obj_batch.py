...
Updated for Python 3 by: Joe Strout <joe@strout.net>
"""
import os
import logging
...
class Material(graphics.Group):
    ...
    def __str__(self):
        return "diffuse:%s ambient%s specular%s emission%s shininess:%s opacity:%s texture:%s" % (self.diffuse, self.ambient, self.specular, self.emission, self.shininess, self.opacity, self.texture)
...
class OBJ(object):
    ...
    def __init__(self, filename, file=None, path=None):
        ...
        for line in file:
            ...
            if values[0] == 'v':
                vertices.append(lmap(float, values[1:4]))
            elif values[0] == 'vn':
                normals.append(lmap(float, values[1:4]))
            elif values[0] == 'vt':
                tex_coords.append(lmap(float, values[1:3]))
                ...
                for i, v in enumerate(values[1:]):
                    v_index, t_index, n_index = \
                        (lmap(int, [j or 0 for j in v.split('/')]) + [0, 0])[:3]
                    if v_index < 0:
    ...
    def load_material_library(self, filename):
        ...
        for line in file:
            ...
            try:
                if values[0] == 'Kd':
                    material.diffuse = lmap(float, values[1:])
                elif values[0] == 'Ka':
                    material.ambient = lmap(float, values[1:])
                elif values[0] == 'Ks':
                    material.specular = lmap(float, values[1:])
                elif values[0] == 'Ke':
                    material.emissive = lmap(float, values[1:])
                elif values[0] == 'Ns':
                    ...
                    try:
                        material.texture = pyglet.resource.texture(os.path.join(self.path,values[1]))
